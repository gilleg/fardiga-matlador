    ...
    <main>
      <article className="article-shell">
        ...
        <div className="article-layout"><div className="article-body">
          ...
          {article.sections.map((section, index) => <section key={section.title}><h2>{section.title}</h2>{index === 1 && <SectionIcon index={0} />}{section.paragraphs?.map((paragraph) => <p key={paragraph}><ExplainedText text={paragraph} /></p>)}{section.bullets && <ul>{section.bullets.map((item) => <li key={item}><ExplainedText text={item} /></li>)}</ul>}
